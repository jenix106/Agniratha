...
namespace Agniratha
{
    class XCRPGModule : LevelModule
    {
        ...
        {
            battleMusic = level.customReferences[0].transforms.Find(match => match.name == "BattleMusic").GetComponent<AudioSource>();
            uniqueBattleMusic = level.customReferences[0].transforms.Find(match => match.name == "UniqueBattleMusic").GetComponent<AudioSource>();
